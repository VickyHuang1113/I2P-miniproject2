...
{
    if(lives<=0) Engine::GameEngine::GetInstance().ChangeScene("lose");
    ...
    for (auto& it : EnemyGroup->GetObjects()) reachEndTimes.push_back(dynamic_cast<Enemy*>(it)->reachEndTime);
    ...
    for (int i = 0; i < SpeedMult; i++)
    {
        ...
        if (ticks < std::get<1>(current)) continue;
        ticks -= std::get<1>(current);
        enemyWaveData.pop_front();
        std::random_device dev;
        std::mt19937 rng(dev());
        std::shuffle(laneNum.begin(), laneNum.end(), rng);
        std::uniform_int_distribution<std::mt19937::result_type> lane_dist(1, 3);

        int ScreenW = Engine::GameEngine::GetInstance().GetScreenSize().x;
        std::random_device dev2;
        std::mt19937 rng2(dev2());
        std::uniform_int_distribution<std::mt19937::result_type> rp_dist(ScreenW/2, ScreenW);

        Enemy* enemy;
        for(int j = 0 ; j < std::get<2>(current) ; j++)
        {
            const Engine::Point SpawnCoordinate = Engine::Point(SpawnGridPointx * BlockSize + BlockSize / 2 + lane_dist(rng) * 15, laneNum[j] * BlockSize + BlockSize / 2);
...
{
    LifeChange(-1);
    //Moved to Update, so as to ensure "change scene" will be update first, and then draw the new scene.
    //(The order "Update and Draw" is defined in Game Engine.
    //So the deleted pointer won't be used at drawing anymore.          ???
    /*if (lives <= 0)
    {
...
void PlayScene::LifeChange(int lives)
{
    this->lives += lives;
...
void PlayScene::ConstructUI()
{
    ...
    //Only called once at PlayScene::Initialized, and can't update the value of money and lives by above two lines.
    //So we need to update them by UIMoney/Lives->Text = ..... somewhere else. (In function EarnMoney and LifeChange)
    ...
    btn = new TurretButton("play/floor.png", "play/dirt.png",
                           ...
                           , 410, 128*MapHeight, WBCellTurret::Price);
